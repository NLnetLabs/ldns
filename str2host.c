...
ldns_str2rdf_nsec3_vars(ldns_rdf **rd, const char *nsec3_vars)
{
	...
	if (opt_in_int > 0) {
		data[1] |= LDNS_NSEC3_VARS_OPTOUT_MASK;
	}
...
ldns_status
ldns_str2rdf_nsec3_params_vars(ldns_rdf **rd, const char *nsec3_vars)
{
	unsigned int algorithm;
	unsigned int iterations;
	uint8_t salt_length;
	...
	pos = sscanf(nsec3_vars, "%u %u %s", &algorithm, &iterations, salt_str);
