...
{
	add_rr_list_to_referrals(referrals, ldns_pkt_all_noquestion(p));
}
		...
		ldns_rr_class c)
{
	...
	while(status == LDNS_STATUS_OK && 
		...
		if (ldns_rr_list_rr_count(
		    ldns_pkt_rr_list_by_type(p, t, LDNS_SECTION_ANSWER)) > 0)
			break;
