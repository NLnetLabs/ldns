...
ldns_lookup_table ldns_hashes[] = {
        {LDNS_SHA1     , "SHA-1" },      /* RFC 4034 */
        {LDNS_SHA256   , "SHA-256" },    /* RFC 4509 */
        {LDNS_HASH_GOST, "HASH-GOST" }, /* RFC 5933 */
        {LDNS_SHA384   , "SHA-384" },    /* RFC 6605 */
        { 0, NULL }
};
...
/* print EDNS option data as a single hex string */
static void
...
ldns_edns_ul2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	ldns_buffer_printf(output, "%lu", (unsigned long)lease);
...
ldns_edns_d3u2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	ldns_buffer_printf(output, "; N3U=");

	for(i=0; i<len; i++) {
		if(data[i] == 1) {
			ldns_buffer_printf(output, " SHA-1");
		} else {
...
ldns_edns_subnet2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	ldns_buffer_printf(output, "/%d/%d", (int)source, (int)scope);
...
ldns_edns_expire2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	decimal = ldns_read_uint32(data);
	ldns_buffer_printf(output, "%lu", (unsigned long) decimal);
...
ldns_edns_padding2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	/* find out if the data consists of all zeroes */
	for (i = 0; i < len; i++) {
		if (data[i] != 0) {
			zeroes = false;
		}
	...
	if (zeroes) {
		ldns_buffer_printf(output, "%d", len);
	} else {
		ldns_edns_hex_data2buffer_str(output, data, len);
	}
...
ldns_edns_chain2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	/* parse the FQDN in the chain */
	if (ldns_wire2dname(&rdf, data, len, &zero) != LDNS_STATUS_OK ||
		ldns_rdf2buffer_str_dname(output, rdf) != LDNS_STATUS_OK) {
...
ldns_edns_key_tag2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	ldns_buffer_printf(output, "; KEYTAG=");
...
ldns_edns_ede2buffer_str(ldns_buffer* output, uint8_t* data, size_t len)
{
	...
	/* skip the EDE code and length in the output */
	data += 4;
	len -= 4;
